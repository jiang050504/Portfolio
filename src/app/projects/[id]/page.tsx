    ...
    <div className="min-h-screen pt-24 pb-16">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {images.map((img, i) => (
                  <div key={i} onClick={() => setFullscreenIdx(i)}
                    ...
                    <img src={img} alt={`${project.title} 截图 ${i + 1}`} className="aspect-video w-full object-cover" />
                  </div>
                ))}
              </div>
            </div>
                ...
                {videos.map((vid, i) => (
                  <div key={i} className="overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02]">
                    {vid.includes("youtube.com") || vid.includes("youtu.be") ? (
                      ...
                      <video src={vid} controls className="aspect-video w-full object-cover" />
                    )}
                  </div>
                ))}
              </div>
      ...
      {fullscreenIdx !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm" onClick={() => setFullscreenIdx(null)}>
          ...
          <img src={images[fullscreenIdx]} alt="" className="max-h-[85vh] max-w-[90vw] rounded-lg object-contain" />
